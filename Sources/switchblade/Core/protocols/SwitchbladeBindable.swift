...
public protocol SwitchbadeBinder {
    ...
    @discardableResult func bind<T: Codable>(key: KeyType,_ onChange: ((T?)->Void)?) -> SWBinding<T>
    @discardableResult func bind<T: Codable>(_ object: T,_ onChange: ((T?)->Void)?) -> SWBinding<T>
    @discardableResult func bind<T: Codable>(key: KeyType, keyspace: String,_ onChange: ((T?)->Void)?) -> SWBinding<T>
