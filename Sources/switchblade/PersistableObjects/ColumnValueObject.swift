...
public class Value {
    ...
    public init(_ value: Any?) {
        ...
        if mirror.subjectType == String.self || mirror.subjectType == String?.self {
        ...
        } else if (mirror.subjectType == [UInt8].self || mirror.subjectType == [UInt8]?.self) {
            type = .Blob
            blobValue = Data(value as! [UInt8])
            ...
            type = .String
            stringValue = value as! String
        } else if (mirror.subjectType == UUID.self || mirror.subjectType == UUID?.self) {
            type = .UUID
            uuidValue = value as! UUID
        } else {
            type = .Null
        }
    }
